...
const productDetails = async (req, res) => {
    try {
        ...
        const cart_products = await cartModel.find({ customerId: userData._id })
        const categories = await categoryModel.find()
        const wish_list = await wishListModel.findOne({ userID: userData._id })
...
const showAllProducts = async (req, res) => {
    try {
        ...
        const wish_list = await wishListModel.findOne({ userID: user_id })
        let perpage = 5
